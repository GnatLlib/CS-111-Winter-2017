...
void print_usage(int target)
{
	
	if(getrusage(target, &usage)<0){
		fprintf(stderr, "Error retrieving usage");
		exit(errno);
	}
	 else {

        double u_time = ((double)usage.ru_utime.tv_sec + (double)usage.ru_utime.tv_usec/1000000) - prev_usrtime;
        double s_time = ((double)usage.ru_stime.tv_sec + (double)usage.ru_stime.tv_usec/1000000) - prev_systime;
        printf("\tCurrent Option Usage Times:\n");
        printf("\t\tUser time:\t%f seconds\n", u_time);
        printf("\t\tSystem time:\t%f seconds\n", s_time);
           
        printf("\tOverall Usage Times:\n");
	...
	while(1){
		if(prof_flag){
                if(getrusage(RUSAGE_SELF, &usage) == -1){
                    ...
                    prev_usrtime = (double)usage.ru_utime.tv_sec + (double)usage.ru_utime.tv_usec/1000000;
                    prev_systime = (double)usage.ru_stime.tv_sec + (double)usage.ru_stime.tv_usec/1000000;
                } 
            }
		...
		switch(opt){
			...
			case 'w':
				...
				if (prof_flag){
						printf("Usage info for file opening: \n");
			...
			case 'p':{
				...
				if (prof_flag){
						printf("Usage info for pipe creation: \n");
                        print_usage(RUSAGE_SELF);
                    }
			...
			case 'x':{
				...
				fds[fd_close] = -1;
				if (prof_flag){
						printf("Usage info for file closing");
                        print_usage(RUSAGE_SELF);
                    }
			...
			case 'W':{
				...
				if (prof_flag){
						printf("Usage Info for Parents: \n");
                        print_usage(RUSAGE_SELF);
						printf("Usage Info for Children: \n");
